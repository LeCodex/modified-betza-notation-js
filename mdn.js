...
function Identifier(identifier, result) {
	...
	if (!match) { result.error = "Could not match the Identifier `" + identifier + "`"; return result; }
...
function MacroIdentifier(macroIdentifier, result) {
	...
	if (!match) { result.error = "Could not match the MacroIdentifier `" + macroIdentifier + "`"; return result; }
...
function PieceIdentifier(pieceIdentifier, result) {
	...
	if (!match) { result.error = "Could not match the PieceIdentifier `" + pieceIdentifier + "`"; return result; }
...
function PieceLetter(pieceLetter, result) {
	...
	if (/^[A-Za-z]$|^:[A-Za-z][A-Za-z0-9]?[A-Za-z0-9]?:$/.test(pieceLetter)) {
		...
		result.error = "Could not match the PieceLetter `" + pieceLetter + "`"; return result;
	};
}
...
function Suffix(suffix, result) {
	...
	if (!match) { result.error = "Could not match the Suffix `" + suffix + "`"; return result; }
...
function FourDBoardIdentifier2(fourDBoardIdentifier, result) {
	...
	if (!match) { result.error = "Could not match the BoardIdentifier `" + fourDBoardIdentifier + "`"; return result; } // Small issue: there is no way of knowing where the error as precisely as the other parts

	var numbers1 = BoardNumbers(match.group.BoardNumbers1);
	var numbers2 = BoardNumbers(match.group.BoardNumbers2);

	if (!numbers1 || !number2) { result.error = "Could not match the BoardNumbers of the BoardIdentifier `" + fourDBoardIdentifier + "`"; return result; }
...
function CombinedMove(combinedMove, result) {
	// <CombinedMove>        ::= <PartialMove> ( '+' <PartialMove> )*
	if (!/^\S+(\+?\S+)*$/.test(combinedMove)) { result.error = "Could not match the CombinedMove `" + combinedMove + "`"; return result; } // Redundant, since all characters non-whitespace already

	var partialMoves = combinedMove.matchAll(/\+?(\S+?)(?=(?<![<\[][a-zA-Z0-9]*)\+)|\+?(\S+)/g) // Matches and separates all the partial moves.

	if (!partialMoves) { result.error = "Could not match the PartialMoves of the CombinedMove `" + combinedMove + "`"; return result; } // Again, redundant
...
function MovePrefix(movePrefix, result, range) {
	// <MovePrefix>          ::= '!'? <BoardTransition>? <Condition>?
	var prefix = {};
	var match = movePrefix.match(/^(?<Setup>!)?(?<BoardTransition><[a-zA-H0-9\-,^]+>)?(?<Condition>\/[{}A-Za-z0-9\-,^]+\/)?$/);

	if (!match) { result.error = "Could not match the MovePrefix `" + movePrefix + "`"; return; }

	if (range) prefix.range = range;
	if (match.groups.Setup) prefix.setup = true;
...
function Moves(moves, result) {
	...
	if (!individualMoves) { result.error = "Could not match the Moves of the Moves `" + moves + "`"; return []; } // Again, redundant
...
function Steps(steps, result) {
	...
	if (!match) { result.error = "Could not match the Steps `" + steps + "`"; return; }
...
function BoardTransition3(boardTransition, result) {
	...
	if (!match) { result.error = "Could not match the BoardTransition `" + boardTransition + "`"; return; }
...
function SingleTransition(singleTransition, result) {
	...
	if (!match) { result.error = "Could not match the SingleTransition `" + singleTransition + "`"; return; }
...
function Transition(transition, result) {
	...
	if (!match) { result.error = "Could not match the Transition `" + transition + "`"; return; }
...
function Modifications(modifications, result) {
	...
	for (var match of individualModifications) {
		...
		if (match.groups.DirectionalModifier) {
			modification.modifier = developModifiers ? Modifier(match.groups.DirectionalModifier, result) : match.groups.DirectionalModifier;
		} else if (match.groups.Modifier) {
			modification.modifier = developModifiers ? Modifier(match.groups.Modifier, result) : match.groups.Modifier;
...
function Modifier(modifier, result) {
	...
	if (table[modifier]) {
		...
		result.error = "Could not match Modifier `" + modifier + "`"; return;
	}
}
...
function SingleRange(singleRange, result) {
	...
	if (!range.file && !range.rank) { result.error = "Could not match the FileRange or RankRange or SquareRange of the SingleRange `" + singleRange + "`"; return; }
...
function RankRange(rankRange, result) {
	...
	if (!match) { result.error = "Could not match the RankRange `" + rankRange + "`"; return; }
...
function ThreeDRange(threeDRange, result) {
	...
	if (!match) { result.error = "Could not match the ThreeDRange `" + threeDRange + "`"; return; }
...
function FourDRange(fourDRange, result) {
	...
	if (!match) { result.error = "Could not match the FourDRange `" + fourDRange + "`"; return; }
...
function PieceSet(pieceSet, result) {
	...
	if (!pieces) { result.error = "Could not match the Pieces of the PieceSet `" + pieceSet + "`"; return; }
...
function Condition(condition, result) {
	...
	if (!match) { result.error = "Could not match the Condition `" + condition + "`"; return; }
...
function Square(square, result) {
	...
	if (!match) { result.error = "Could not match the Square `" + square + "`"; return; }
